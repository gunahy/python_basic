...
numbers = [2, -5, 8, 9, -25, 25, 4, 78, 256, 625, 789]
result = []
...
try:
    ...
    print(f'Данной даты быть не может. Ключ {e} отсутствует в списке')
...
simle_list = [1 , 2, 4, 5, 6, 2, 5, 2]
once_numbers = []
unique_numbers = []

for item in simle_list:
    count = simle_list.count(item)
    if simle_list.count(item) == 1: 
        unique_numbers.append(item)
    if once_numbers.count(item) == 1:
        continue
    once_numbers.append(item)
